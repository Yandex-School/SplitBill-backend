...
class GetCreatedRooms final : public userver::server::handlers::HttpHandlerBase {
 public:
  static constexpr std::string_view kName = "handler-v1-get-created-rooms";

  GetCreatedRooms(const userver::components::ComponentConfig& config,
  ...
  std::string HandleRequestThrow(
    ...
    auto count_result = pg_cluster_->Execute(
        userver::storages::postgres::ClusterHostType::kMaster,
        "SELECT COUNT(*) FROM rooms r WHERE r.user_id = $1",
        user_id
    );
    int total_count = count_result.Front()[0].As<int>();

    try {
      // Query rooms for this user with pagination and sorting
...
void AppendGetCreatedRooms(userver::components::ComponentList& component_list) {
  component_list.Append<GetCreatedRooms>();
}
